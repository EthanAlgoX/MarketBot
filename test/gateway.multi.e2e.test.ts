...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `marketbot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".marketbot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MARKETBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        MARKETBOT_GATEWAY_TOKEN: inst.gatewayToken,
        MARKETBOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          MARKETBOT_GATEWAY_PORT: String(gwA.port),
          MARKETBOT_GATEWAY_TOKEN: gwA.gatewayToken,
          MARKETBOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          MARKETBOT_GATEWAY_PORT: String(gwB.port),
          MARKETBOT_GATEWAY_TOKEN: gwB.gatewayToken,
          MARKETBOT_GATEWAY_PASSWORD: "",
        }),
