...
import type { MarketBotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("marketbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("marketbot-bin-sherpa-");
      const modelDir = await makeTempDir("marketbot-sherpa-model-");
      ...
      const cfg: MarketBotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("marketbot-bin-whispercpp-");
      const modelDir = await makeTempDir("marketbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("marketbot-bin-gemini-");
      ...
      const cfg: MarketBotConfig = { tools: { media: { image: {} } } };
