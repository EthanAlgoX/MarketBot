...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MARKETBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.MARKETBOT_STATE_DIR = path.join(base, ".marketbot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "marketbot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".marketbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
