...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "logs") {
    ...
    await loadLogs(host as unknown as MarketBotApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__MARKETBOT_CONTROL_UI_BASE_PATH__;
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MarketBotApp, true),
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as MarketBotApp, false),
    loadCronStatus(host as unknown as MarketBotApp),
    loadCronJobs(host as unknown as MarketBotApp),
  ]);
}
