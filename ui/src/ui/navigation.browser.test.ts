...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/marketbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/marketbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/marketbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MARKETBOT_CONTROL_UI_BASE_PATH__ = "/marketbot";
    const app = mountApp("/marketbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/marketbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/marketbot/sessions");
  });
