...
import Foundation
import Testing
@testable import MarketBotChatUI
...
private final class TestChatTransport: @unchecked Sendable, MarketBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MarketBotChatHistoryPayload]
    private let sessionsResponses: [MarketBotChatSessionsListResponse]

    private let stream: AsyncStream<MarketBotChatTransportEvent>
    private let continuation: AsyncStream<MarketBotChatTransportEvent>.Continuation

    init(
        historyResponses: [MarketBotChatHistoryPayload],
        sessionsResponses: [MarketBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MarketBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MarketBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MarketBotChatHistoryPayload(
        ...
        attachments _: [MarketBotChatAttachmentPayload]) async throws -> MarketBotChatSendResponse
    {
        ...
        return MarketBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MarketBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MarketBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MarketBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MarketBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MarketBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MarketBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MarketBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MarketBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MarketBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MarketBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
