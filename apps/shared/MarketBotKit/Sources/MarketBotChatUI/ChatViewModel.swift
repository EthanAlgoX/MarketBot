...
private let chatUILogger = Logger(subsystem: "ai.marketbot", category: "MarketBotChatUI")

@MainActor
@Observable
public final class MarketBotChatViewModel {
    public private(set) var messages: [MarketBotChatMessage] = []
    ...
    public var attachments: [MarketBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MarketBotChatPendingToolCall] = []
    public private(set) var sessions: [MarketBotChatSessionEntry] = []
    private let transport: any MarketBotChatTransport
    ...
    private var pendingToolCallsById: [String: MarketBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MarketBotChatTransport) {
    ...
    public var sessionChoices: [MarketBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [MarketBotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [MarketBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MarketBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MarketBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MarketBotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [MarketBotChatMessage]) -> [MarketBotChatMessage] {
        var result: [MarketBotChatMessage] = []
    ...
    private static func dedupeKey(for message: MarketBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MarketBotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> MarketBotChatAttachmentPayload in
            MarketBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MarketBotChatSessionEntry {
        MarketBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MarketBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MarketBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MarketBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MarketBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MarketBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
