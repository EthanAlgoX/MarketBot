...
struct ChatMessageBubble: View {
    let message: MarketBotChatMessage
    let style: MarketBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? MarketBotChatTheme.userText : MarketBotChatTheme.assistantText
    ...
    private var inlineAttachments: [MarketBotChatMessageContent] {
    ...
    private var toolCalls: [MarketBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [MarketBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? MarketBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return MarketBotChatTheme.onboardingAssistantBubble
        }
        return MarketBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return MarketBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: MarketBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? MarketBotChatTheme.userText : MarketBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: MarketBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(MarketBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(MarketBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [MarketBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: MarketBotChatTheme.assistantText)
            }
        }
    }
}
