...
@MainActor
public struct MarketBotChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: MarketBotChatViewModel
    ...
    public init(
        viewModel: MarketBotChatViewModel,
    ...
    private var visibleMessages: [MarketBotChatMessage] {
        let base: [MarketBotChatMessage]
    ...
    private func mergeToolResults(in messages: [MarketBotChatMessage]) -> [MarketBotChatMessage] {
        ...
        for message in messages {
            ...
            let merged = MarketBotChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: MarketBotChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: MarketBotChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: MarketBotChatMessage) -> String {
