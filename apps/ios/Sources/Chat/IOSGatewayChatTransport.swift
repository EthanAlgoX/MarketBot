...
struct IOSGatewayChatTransport: MarketBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MarketBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MarketBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MarketBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MarketBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MarketBotChatAttachmentPayload]) async throws -> MarketBotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MarketBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(MarketBotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(MarketBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MarketBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MarketBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MarketBotAgentEventPayload.self)
                        {
