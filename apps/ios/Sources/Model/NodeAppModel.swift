...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = MarketBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = MarketBotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = MarketBotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = MarketBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = MarketBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__marketbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: MarketBotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for marketbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case MarketBotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MarketBotCanvasCommand.present.rawValue,
                 MarketBotCanvasCommand.hide.rawValue,
                 MarketBotCanvasCommand.navigate.rawValue,
                 MarketBotCanvasCommand.evalJS.rawValue,
                 MarketBotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case MarketBotCanvasA2UICommand.reset.rawValue,
                 MarketBotCanvasA2UICommand.push.rawValue,
                 MarketBotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case MarketBotCameraCommand.list.rawValue,
                 MarketBotCameraCommand.snap.rawValue,
                 MarketBotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case MarketBotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: MarketBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: MarketBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(MarketBotLocationGetParams.self, from: req.paramsJSON)) ??
            MarketBotLocationGetParams()
        ...
        let payload = MarketBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case MarketBotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(MarketBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                MarketBotCanvasPresentParams()
        ...
        case MarketBotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MarketBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(MarketBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case MarketBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(MarketBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarketBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(MarketBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case MarketBotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              ...
              if (!host) return JSON.stringify({ ok: false, error: "missing marketbotA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case MarketBotCanvasA2UICommand.push.rawValue, MarketBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == MarketBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(MarketBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try MarketBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(MarketBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try MarketBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.marketbotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case MarketBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarketBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(MarketBotCameraSnapParams.self, from: req.paramsJSON)) ??
                MarketBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case MarketBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(MarketBotCameraClipParams.self, from: req.paramsJSON)) ??
                MarketBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(MarketBotScreenRecordParams.self, from: req.paramsJSON)) ??
            MarketBotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> MarketBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return MarketBotLocationMode(rawValue: raw) ?? .off
    }
