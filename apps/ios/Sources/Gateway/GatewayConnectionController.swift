...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [MarketBotCapability.canvas.rawValue, MarketBotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(MarketBotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(MarketBotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = MarketBotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(MarketBotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            MarketBotSystemCommand.notify.rawValue,
            MarketBotSystemCommand.which.rawValue,
            MarketBotSystemCommand.run.rawValue,
            MarketBotSystemCommand.execApprovalsGet.rawValue,
            MarketBotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(MarketBotCapability.camera.rawValue) {
            commands.append(MarketBotCameraCommand.list.rawValue)
            commands.append(MarketBotCameraCommand.snap.rawValue)
            commands.append(MarketBotCameraCommand.clip.rawValue)
        }
        if caps.contains(MarketBotCapability.location.rawValue) {
            commands.append(MarketBotLocationCommand.get.rawValue)
        }
