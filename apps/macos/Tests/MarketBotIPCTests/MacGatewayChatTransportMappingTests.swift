...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: MarketBotProtocol.AnyCodable(["ok": MarketBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: MarketBotProtocol.AnyCodable(["ok": MarketBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = MarketBotProtocol.AnyCodable([
            "runId": MarketBotProtocol.AnyCodable("run-1"),
            "sessionKey": MarketBotProtocol.AnyCodable("main"),
            "state": MarketBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: MarketBotProtocol.AnyCodable(["a": MarketBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
