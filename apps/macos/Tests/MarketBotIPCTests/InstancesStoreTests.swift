...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MarketBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MarketBotProtocol.AnyCodable] = [
            "presence": .init([MarketBotProtocol.AnyCodable(entry)]),
        ]
        let payload = MarketBotProtocol.AnyCodable(payloadMap)
