...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.marketbot/marketbot.json. Prefer MARKETBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.marketbot/marketbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.marketbot/marketbot.json. Restart the gateway to apply."
        }
