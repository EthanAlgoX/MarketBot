...
enum CommandResolver {
    private static let projectRootDefaultsKey = "marketbot.gatewayProjectRootPath"
    private static let helperName = "marketbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let marketbotEntry = root.appendingPathComponent("marketbot.mjs").path
        if FileManager().isReadableFile(atPath: marketbotEntry) { return marketbotEntry }
        let binEntry = root.appendingPathComponent("bin/marketbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/marketbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let marketbotPaths = self.marketbotManagedPaths(home: home)
        if !marketbotPaths.isEmpty {
            extras.insert(contentsOf: marketbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + marketbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func marketbotManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".marketbot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func marketbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMarketBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("marketbot.mjs").path,
            root.appendingPathComponent("bin/marketbot.js").path,
        ]
    ...
    static func hasAnyMarketBotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.marketbotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func marketbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let marketbotPath = self.projectMarketBotExecutable(projectRoot: root) {
                return [marketbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "marketbot", subcommand] + extraArgs
            }
            if let marketbotPath = self.marketbotExecutable(searchPaths: searchPaths) {
                return [marketbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            marketbot entrypoint missing (looked for dist/index.js or marketbot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real marketbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/marketbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v marketbot >/dev/null 2>&1; then
          CLI="$(command -v marketbot)"
          marketbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/marketbot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/marketbot.mjs"
            node "$PRJ/marketbot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/marketbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/marketbot.js"
            node "$PRJ/bin/marketbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent marketbot"
          pnpm --silent marketbot \(quotedArgs);
        else
          echo "marketbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MarketBotConfigFile.loadDict()
