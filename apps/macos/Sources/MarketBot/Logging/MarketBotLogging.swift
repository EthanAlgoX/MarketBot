...
enum MarketBotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = MarketBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MarketBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MarketBotLogging.bootstrapIfNeeded()
        let label = MarketBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MarketBotOSLogHandler: LogHandler {
...
struct MarketBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MarketBotLogging.parseLabel(self.label)
