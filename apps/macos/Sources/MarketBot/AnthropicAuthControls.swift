...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MarketBotOAuthStore.AnthropicOAuthStatus = MarketBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MarketBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MarketBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MarketBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try MarketBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. MarketBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MarketBotOAuthStore.AnthropicOAuthStatus,
