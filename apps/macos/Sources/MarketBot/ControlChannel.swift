...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MarketBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.marketbot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MarketBotKit.AnyCodable]()) {
                $0[$1.key] = MarketBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MarketBotProtocol.AnyCodable?) -> [String: MarketBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MarketBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MarketBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MarketBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("marketbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("marketbot.control.agent")
}
