...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MARKETBOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/marketbot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("marketbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("marketbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/marketbot/ (rolling or stdout), or nil if none exist.
