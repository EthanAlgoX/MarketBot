...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.marketbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MarketBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MarketBotSessionPreviewEntry,
    ...
    private static func cache(payload: MarketBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MarketBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MarketBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MarketBotKit.AnyCodable]) -> [MarketBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MarketBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MarketBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MarketBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MarketBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MarketBotChatMessage) -> String? {
