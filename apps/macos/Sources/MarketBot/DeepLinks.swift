...
private let deepLinkLogger = Logger(subsystem: "ai.marketbot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "MarketBot is paused", message: "Unpause MarketBot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run MarketBot agent?", message: body) else { return }
        }
