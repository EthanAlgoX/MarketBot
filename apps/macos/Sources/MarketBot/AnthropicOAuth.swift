...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (MarketBot token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: MarketBotOAuthStore.AnthropicOAuthStatus = MarketBotOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.marketbot", category: "anthropic-oauth")
...
enum MarketBotOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let marketbotOAuthDirEnv = "MARKETBOT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "MarketBot OAuth token file not found"
            case .unreadableFile: "MarketBot OAuth token file not readable"
            case .invalidJSON: "MarketBot OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in MarketBot OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "MarketBot OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.marketbotOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".marketbot", isDirectory: true)
