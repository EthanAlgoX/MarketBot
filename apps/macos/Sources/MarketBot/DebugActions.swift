...
enum DebugActions {
    private static let verboseDefaultsKey = "marketbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "marketbot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MarketBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "MarketBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MarketBotPaths.configURL
        guard
