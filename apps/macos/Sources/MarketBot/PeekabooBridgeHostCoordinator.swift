...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.marketbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MarketBotPeekabooBridgeServices?
    private static var marketbotSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("MarketBot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MarketBotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.marketbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.marketbotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MarketBotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.marketbot.peekaboo")
