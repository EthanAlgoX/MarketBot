...
private let webChatSwiftLogger = Logger(subsystem: "ai.marketbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: MarketBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> MarketBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(MarketBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> MarketBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                MarketBotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    MarketBotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    MarketBotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<MarketBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any MarketBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = MarketBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: MarketBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<MarketBotChatView>) -> NSViewController
    {
