...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.marketbot", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous MarketBot run already has an SSH listener on the expected port (common after restarts),
