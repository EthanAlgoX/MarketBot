# MarketBot ACP Bridge

This document describes how the MarketBot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`marketbot acp` exposes an ACP agent over stdio and forwards prompts to a running
MarketBot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a MarketBot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `marketbot acp` over stdio.

Example config:

```bash
marketbot config set gateway.remote.url wss://gateway-host:18789
marketbot config set gateway.remote.token <token>
```

Example run:

```bash
marketbot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
marketbot acp --session agent:main:main
marketbot acp --session agent:design:main
marketbot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “MarketBot ACP” to start a thread.

## Execution Model

- ACP client spawns `marketbot acp` and speaks ACP messages over stdio.
...
```bash
marketbot acp --session agent:main:main
marketbot acp --session-label "support inbox"
marketbot acp --reset-session
```
...
`marketbot acp` resolves the Gateway URL and auth from CLI flags or config:
