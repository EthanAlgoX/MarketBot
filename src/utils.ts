...
export function resolveConfigDir(
  ...
  const override = env.MARKETBOT_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(homedir(), ".marketbot");
  try {
...
// Configuration root; can be overridden via MARKETBOT_STATE_DIR.
