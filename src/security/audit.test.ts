...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "marketbot.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "marketbot.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: MarketBotConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-discord-"));
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "marketbot-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.MARKETBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.MARKETBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-discord-open-"));
    process.env.MARKETBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MarketBotConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.MARKETBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-slack-"));
    process.env.MARKETBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MarketBotConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.MARKETBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-slack-open-"));
    process.env.MARKETBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MarketBotConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.MARKETBOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-telegram-"));
    process.env.MARKETBOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: MarketBotConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: MarketBotConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.MARKETBOT_GATEWAY_TOKEN;
    process.env.MARKETBOT_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: MarketBotConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.marketbot",
      configPath: "/Users/test/Dropbox/.marketbot/marketbot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: MarketBotConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "marketbot.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.MARKETBOT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.MARKETBOT_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.MARKETBOT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.MARKETBOT_GATEWAY_PASSWORD;
      } else {
        process.env.MARKETBOT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.MARKETBOT_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: MarketBotConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.MARKETBOT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: MarketBotConfig = {
        gateway: {
          mode: "remote",
