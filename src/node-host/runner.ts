...
const execHostEnforced = process.env.MARKETBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.MARKETBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureMarketBotCliOnPath({ pathEnv: process.env.PATH ?? "" });
