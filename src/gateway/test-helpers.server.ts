...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.MARKETBOT_STATE_DIR;
  previousConfigPath = process.env.MARKETBOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.MARKETBOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.MARKETBOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.MARKETBOT_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.MARKETBOT_STATE_DIR = path.join(tempHome, ".marketbot");
  delete process.env.MARKETBOT_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.MARKETBOT_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "marketbot-test-"))
    : path.join(tempHome, ".marketbot-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.MARKETBOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.MARKETBOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.MARKETBOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.MARKETBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MARKETBOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.MARKETBOT_SKIP_CANVAS_HOST;
    } else {
      process.env.MARKETBOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.MARKETBOT_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MARKETBOT_GATEWAY_TOKEN = fallbackToken;
  }
