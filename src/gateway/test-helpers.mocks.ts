...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `marketbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.MARKETBOT_CONFIG_PATH = path.join(root, "marketbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "marketbot.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "marketbot-gateway-test"),
        ...fileDefaults,
