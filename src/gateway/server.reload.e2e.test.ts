...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.MARKETBOT_SKIP_CHANNELS;
    prevSkipGmail = process.env.MARKETBOT_SKIP_GMAIL_WATCHER;
    process.env.MARKETBOT_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.MARKETBOT_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.MARKETBOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MARKETBOT_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
