...
export function resolveHooksConfig(cfg: MarketBotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-marketbot-token"] === "string"
      ? req.headers["x-marketbot-token"].trim()
      : "";
  if (headerToken) {
