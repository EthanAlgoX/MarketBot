    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "marketbot", messages: [{ role: "user", content: "hi" }] },
          { "x-marketbot-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-marketbot-agent-id": "beta",
            "x-marketbot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
