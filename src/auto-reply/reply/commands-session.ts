...
import { scheduleGatewaySigusr1Restart, triggerMarketBotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting MarketBot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMarketBotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting MarketBot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
