...
const prevBundledDir = process.env.MARKETBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `marketbot-plugin-${randomUUID()}`);
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.MARKETBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadMarketBotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.MARKETBOT_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadMarketBotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
