...
import { discoverMarketBotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MarketBotPluginDefinition;
  register?: MarketBotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MarketBotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MarketBotPluginDefinition;
...
export function loadMarketBotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverMarketBotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "marketbot/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MarketBotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as MarketBotPluginModule;
    } catch (err) {
