...
const SEARCH_TOOL = "https://docs.marketbot.ai/mcp.SearchMarketBot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.marketbot.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('marketbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.marketbot.ai/");
      runtime.log(`Search: ${formatCliCommand('marketbot docs "your query"')}`);
    }
    return;
  }
