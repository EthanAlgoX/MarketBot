...
const requireAgentDir = () => {
  const agentDir = process.env.MARKETBOT_AGENT_DIR;
  if (!agentDir) {
    throw new Error("MARKETBOT_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MARKETBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-oauth-"));
    process.env.MARKETBOT_STATE_DIR = tempStateDir;
    process.env.MARKETBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.MARKETBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MARKETBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-minimax-"));
    process.env.MARKETBOT_STATE_DIR = tempStateDir;
    process.env.MARKETBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.MARKETBOT_AGENT_DIR;
