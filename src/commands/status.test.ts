...
vi.mock("../infra/marketbot-root.js", () => ({
  resolveMarketBotPackageRoot: vi.fn().mockResolvedValue("/tmp/marketbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/marketbot/pnpm-lock.yaml",
      markerPath: "/tmp/marketbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("MarketBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("marketbot status --all") ||
          l.includes("marketbot --profile isolated status --all") ||
          l.includes("marketbot status --all") ||
          l.includes("marketbot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MARKETBOT_GATEWAY_TOKEN = "abcd1234";
    try {
