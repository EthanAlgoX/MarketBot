...
export function listAgentEntries(cfg: MarketBotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: MarketBotConfig, agentId: string) {
...
function resolveAgentModel(cfg: MarketBotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MarketBotConfig): AgentSummary[] {
