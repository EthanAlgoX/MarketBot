...
beforeEach(() => {
  ...
  resolveMarketBotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.MARKETBOT_STATE_DIR;
  originalUpdateInProgress = process.env.MARKETBOT_UPDATE_IN_PROGRESS;
  process.env.MARKETBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketbot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.MARKETBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.MARKETBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.MARKETBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveMarketBotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadMarketBotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/marketbot-root.js", () => ({
  resolveMarketBotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketbot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.MARKETBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when MARKETBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/marketbot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.MARKETBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
