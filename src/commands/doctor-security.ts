...
import type { MarketBotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: MarketBotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("marketbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("marketbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("marketbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("marketbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("marketbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
