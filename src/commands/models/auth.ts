...
import { readConfigFileSnapshot, type MarketBotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: MarketBotConfig, model: string): MarketBotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("marketbot plugins install")}\`.`,
    );
  }
