...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMarketBotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("marketbot models auth setup-token")}\` or \`${formatCliCommand("marketbot configure")}\`.`
          : `Run \`${formatCliCommand("marketbot configure")}\` or set an API key env var.`;
