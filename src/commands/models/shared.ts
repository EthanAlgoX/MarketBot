...
export async function updateConfig(
  mutator: (cfg: MarketBotConfig) => MarketBotConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MarketBotConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MarketBotConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("marketbot agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
