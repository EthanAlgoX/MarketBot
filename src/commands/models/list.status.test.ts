...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveMarketBotAgentDir: vi.fn().mockReturnValue("/tmp/marketbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/marketbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/marketbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveMarketBotAgentDir: mocks.resolveMarketBotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveMarketBotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/marketbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/marketbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/marketbot-agent-custom");
