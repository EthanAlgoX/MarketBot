...
function resolveMode(cfg: MarketBotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("MarketBot doctor");

  const root = await resolveMarketBotPackageRoot({
  ...
  let cfg: MarketBotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("marketbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("marketbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("marketbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("marketbot doctor --fix")}" to apply changes.`);
  }
