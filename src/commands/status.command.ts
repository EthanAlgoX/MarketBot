...
export async function statusCommand(
  ...
  runtime.log(theme.heading("MarketBot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("marketbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("marketbot security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.marketbot.ai/faq");
  runtime.log("Troubleshooting: https://docs.marketbot.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("marketbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("marketbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("marketbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("marketbot gateway probe")}`);
  }
}
