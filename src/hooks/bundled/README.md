...
This directory contains hooks that ship with MarketBot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.marketbot.ai/hooks/soul-evil
...
metadata:
  { "marketbot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.marketbot/marketbot.json`:
...
2. Restart gateway: `pkill -9 -f 'marketbot.*gateway' && pnpm marketbot gateway`
3. Enable the hook: `marketbot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.marketbot.ai/hooks
