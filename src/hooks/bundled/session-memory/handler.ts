...
const saveSessionToMemory: HookHandler = async (event) => {
  ...
  try {
    ...
    const cfg = context.cfg as MarketBotConfig | undefined;
    const agentId = resolveAgentIdFromSessionKey(event.sessionKey);
    const workspaceDir = cfg
      ? resolveAgentWorkspaceDir(cfg, agentId)
      : path.join(os.homedir(), ".marketbot", "workspace");
    ...
    if (sessionFile) {
      ...
      if (sessionContent && cfg) {
        ...
        const marketbotRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
        const slugGenPath = path.join(marketbotRoot, "llm-slug-generator.js");
