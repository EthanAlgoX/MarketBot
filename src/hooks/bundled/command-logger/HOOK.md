...
homepage: https://docs.marketbot.ai/hooks#command-logger
metadata:
  {
    "marketbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.marketbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.marketbot/logs/commands.log ~/.marketbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/marketbot`:
   ```
   /home/username/.marketbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
