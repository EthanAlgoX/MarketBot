...
import type { MarketBotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: MarketBotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: MarketBotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MarketBotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MarketBotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MarketBotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as MarketBotConfig)),
    ),
  };
}
