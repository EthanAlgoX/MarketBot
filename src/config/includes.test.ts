...
const ETC_MARKETBOT_DIR = path.join(ROOT_DIR, "etc", "marketbot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "marketbot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcMarketBotPath(...parts: string[]) {
  return path.join(ETC_MARKETBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcMarketBotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "marketbot.json"))).toEqual({
      shared: true,
    });
  });
});
