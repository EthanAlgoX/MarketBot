...
import type { MarketBotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareMarketBotVersions } from "./version.js";
...
function coerceConfig(value: unknown): MarketBotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as MarketBotConfig;
}
...
function stampConfigVersion(cfg: MarketBotConfig): MarketBotConfig {
...
function warnIfConfigFromFuture(cfg: MarketBotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareMarketBotVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer MarketBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: MarketBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): MarketBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as MarketBotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as MarketBotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: MarketBotConfig) {
    clearConfigCache();
...
// module scope. `MARKETBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.MARKETBOT_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MARKETBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): MarketBotConfig {
...
export async function writeConfigFile(cfg: MarketBotConfig): Promise<void> {
  clearConfigCache();
