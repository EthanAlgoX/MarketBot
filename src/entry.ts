...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.MARKETBOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.MARKETBOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.MARKETBOT_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[marketbot] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[marketbot] ${parsed.error}`);
    process.exit(2);
  }
