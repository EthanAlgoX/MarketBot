...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.MARKETBOT_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.MARKETBOT_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.MARKETBOT_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.MARKETBOT_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
