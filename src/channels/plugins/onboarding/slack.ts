...
function setSlackDmPolicy(cfg: MarketBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "MarketBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for MarketBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: MarketBotConfig, allowFrom: string[]): MarketBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
