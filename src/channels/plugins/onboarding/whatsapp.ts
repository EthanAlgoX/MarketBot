...
function setWhatsAppDmPolicy(cfg: MarketBotConfig, dmPolicy: DmPolicy): MarketBotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: MarketBotConfig, allowFrom?: string[]): MarketBotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: MarketBotConfig, selfChatMode: boolean): MarketBotConfig {
...
async function detectWhatsAppLinked(cfg: MarketBotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so MarketBot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for MarketBot" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("marketbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
