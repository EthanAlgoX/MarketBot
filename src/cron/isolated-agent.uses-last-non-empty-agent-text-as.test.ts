...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "marketbot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".marketbot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<MarketBotConfig> = {},
): MarketBotConfig {
  const base: MarketBotConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".marketbot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
