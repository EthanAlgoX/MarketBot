...
function resolveExecConfig(cfg: MarketBotConfig | undefined) {
...
export function createMarketBotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createMarketBotReadTool(freshReadTool)];
    }
