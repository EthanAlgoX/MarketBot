...
describe("resolveMarketBotAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-agent-"));
    process.env.MARKETBOT_STATE_DIR = tempStateDir;
    delete process.env.MARKETBOT_AGENT_DIR;
    delete process.env.PI_CODING_AGENT_DIR;

    const resolved = resolveMarketBotAgentDir();

    expect(resolved).toBe(path.join(tempStateDir, "agents", "main", "agent"));
  });

  it("honors MARKETBOT_AGENT_DIR overrides", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-agent-"));
    const override = path.join(tempStateDir, "agent");
    process.env.MARKETBOT_AGENT_DIR = override;
    delete process.env.PI_CODING_AGENT_DIR;
