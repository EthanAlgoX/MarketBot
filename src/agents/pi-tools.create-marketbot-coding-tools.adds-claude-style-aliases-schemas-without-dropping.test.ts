...
import { __testing, createMarketBotCodingTools } from "./pi-tools.js";
...
describe("createMarketBotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createMarketBotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "marketbot-outside.txt");
