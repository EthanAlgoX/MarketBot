...
type WebSearchConfig = NonNullable<MarketBotConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MarketBotConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("marketbot configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.marketbot.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://marketbot.ai",
      "X-Title": "MarketBot Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
