...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "marketbot.sandbox=1");
  args.push("--label", `marketbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `marketbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `marketbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("marketbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`marketbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`marketbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("marketbot sandbox recreate --all");
}
