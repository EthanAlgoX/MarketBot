...
/**
 * Cleanup suspended MarketBot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: MarketBotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-cli-images-"));
  const paths: string[] = [];
