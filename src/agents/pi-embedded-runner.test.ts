...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-embedded-agent-"));
...
const ensureModels = (cfg: MarketBotConfig) => ensureMarketBotModelsJson(cfg, agentDir) as unknown;
