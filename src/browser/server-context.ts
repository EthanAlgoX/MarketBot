...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchMarketBotChrome,
  resolveMarketBotUserDataDir,
  stopMarketBotChrome,
} from "./chrome.js";
...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchMarketBotChrome(current.resolved, profile);
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by marketbot. ` +
    ...
    await stopMarketBotChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchMarketBotChrome(current.resolved, profile);
