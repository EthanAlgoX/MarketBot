...
vi.mock("./chrome.js", () => ({
  resolveMarketBotUserDataDir: vi.fn(() => "/tmp/marketbot-test/marketbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveMarketBotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "marketbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveMarketBotUserDataDir).mockReturnValue(userDataDir);
