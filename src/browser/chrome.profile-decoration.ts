...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".marketbot-profile-decorated");
}
 ...
 */
export function decorateMarketBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MARKETBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MARKETBOT_BROWSER_COLOR).toUpperCase();
