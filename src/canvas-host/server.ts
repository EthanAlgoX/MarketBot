...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.marketbotCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.marketbotCanvasA2UIAction &&
        typeof window.marketbotCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.marketbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("marketbot:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android MarketBot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.marketbotSendUserAction === "function"
        ? window.marketbotSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MARKETBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.MARKETBOT_SKIP_CANVAS_HOST)) {
    return true;
...
function resolveDefaultCanvasRoot(): string {
  const candidates = [path.join(os.homedir(), ".marketbot", "canvas")];
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>MarketBot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
