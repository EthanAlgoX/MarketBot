...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `marketbot --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.MARKETBOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.MARKETBOT_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[marketbot] Gateway daemon failed:",
