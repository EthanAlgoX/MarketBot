...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "marketbot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      MARKETBOT_NO_RESPAWN: "1",
      MARKETBOT_STATE_DIR: stateDir,
      MARKETBOT_SKIP_CHANNELS: "1",
      MARKETBOT_SKIP_GMAIL_WATCHER: "1",
      MARKETBOT_SKIP_CRON: "1",
      MARKETBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      MARKETBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "marketbot-entry-bootstrap.mjs");
