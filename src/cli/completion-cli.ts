...
export async function installCompletion(shell: string, yes: boolean, binName = "marketbot") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c marketbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'marketbot gateway' and not 'marketbot other gateway', we need to check the sequence.
