...
const SANDBOX_EXAMPLES = {
  main: [
    ["marketbot sandbox list", "List all sandbox containers."],
    ["marketbot sandbox list --browser", "List only browser containers."],
    ["marketbot sandbox recreate --all", "Recreate all containers."],
    ["marketbot sandbox recreate --session main", "Recreate a specific session."],
    ["marketbot sandbox recreate --agent mybot", "Recreate agent containers."],
    ["marketbot sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["marketbot sandbox list", "List all sandbox containers."],
    ["marketbot sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ...
    ["marketbot sandbox recreate --session main", "Recreate a specific session."],
    [
      "marketbot sandbox recreate --agent mybot",
      "Recreate a specific agent (includes sub-agents).",
    ],
    ["marketbot sandbox recreate --browser --all", "Recreate only browser containers."],
    ["marketbot sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["marketbot sandbox explain", "Show effective sandbox config."],
    ["marketbot sandbox explain --session agent:main:main", "Explain a specific session."],
    ["marketbot sandbox explain --agent work", "Explain an agent sandbox."],
    ["marketbot sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.marketbot.ai/cli/sandbox")}\n`,
    )
    .action(() => {
