...
${formatHelpExamples([
  ['marketbot agent --to +15555550123 --message "status update"', "Start a new session."],
  ['marketbot agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'marketbot agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'marketbot agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['marketbot agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'marketbot agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.marketbot.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.marketbot.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'marketbot agents set-identity --agent main --name "MarketBot" --emoji "📈"',
    "Set name + emoji.",
  ],
  ["marketbot agents set-identity --agent main --avatar avatars/marketbot.png", "Set avatar path."],
  [
    "marketbot agents set-identity --workspace ~/.marketbot/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "marketbot agents set-identity --identity-file ~/.marketbot/workspace/IDENTITY.md --agent main",
