...
const STEP_LABELS: Record<string, string> = {
  ...
  "marketbot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "marketbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const MARKETBOT_REPO_URL = "https://github.com/marketbot/marketbot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".marketbot");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("marketbot@")) {
    return trimmed.slice("marketbot@".length);
  }
...
function resolveGitInstallDir(): string {
  const override = process.env.MARKETBOT_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MARKETBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MARKETBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set MARKETBOT_GIT_DIR to an empty folder or an marketbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MARKETBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("MarketBot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating MarketBot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this MarketBot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("marketbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("marketbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g marketbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g marketbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.MARKETBOT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.MARKETBOT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("marketbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("marketbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("marketbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("marketbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `marketbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `MARKETBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set MARKETBOT_GIT_DIR to an empty folder or an marketbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via MARKETBOT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update MarketBot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["marketbot update", "Update a source checkout (git)"],
        ["marketbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["marketbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["marketbot update --tag beta", "One-off update to a dist-tag or version"],
        ["marketbot update --no-restart", "Update without restarting the service"],
        ["marketbot update --json", "Output result as JSON"],
        ["marketbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["marketbot update wizard", "Interactive update wizard"],
        ["marketbot --update", "Shorthand for marketbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run marketbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.marketbot.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["marketbot update status", "Show channel + version status."],
          ["marketbot update status --json", "JSON output."],
          ["marketbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.marketbot.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
