...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    MARKETBOT_STATE_DIR: process.env.MARKETBOT_STATE_DIR,
    MARKETBOT_CONFIG_PATH: process.env.MARKETBOT_CONFIG_PATH,
    MARKETBOT_GATEWAY_PORT: process.env.MARKETBOT_GATEWAY_PORT,
    MARKETBOT_PROFILE: process.env.MARKETBOT_PROFILE,
  };

  beforeEach(() => {
    process.env.MARKETBOT_STATE_DIR = "/tmp/marketbot-cli-state";
    process.env.MARKETBOT_CONFIG_PATH = "/tmp/marketbot-cli-state/marketbot.json";
  ...
  afterEach(() => {
    if (originalEnv.MARKETBOT_STATE_DIR !== undefined) {
      process.env.MARKETBOT_STATE_DIR = originalEnv.MARKETBOT_STATE_DIR;
    } else {
      delete process.env.MARKETBOT_STATE_DIR;
    }

    if (originalEnv.MARKETBOT_CONFIG_PATH !== undefined) {
      process.env.MARKETBOT_CONFIG_PATH = originalEnv.MARKETBOT_CONFIG_PATH;
    } else {
      delete process.env.MARKETBOT_CONFIG_PATH;
    }

    if (originalEnv.MARKETBOT_GATEWAY_PORT !== undefined) {
      process.env.MARKETBOT_GATEWAY_PORT = originalEnv.MARKETBOT_GATEWAY_PORT;
    } else {
      delete process.env.MARKETBOT_GATEWAY_PORT;
    }

    if (originalEnv.MARKETBOT_PROFILE !== undefined) {
      process.env.MARKETBOT_PROFILE = originalEnv.MARKETBOT_PROFILE;
    } else {
      delete process.env.MARKETBOT_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        MARKETBOT_PROFILE: "dev",
        MARKETBOT_STATE_DIR: "/tmp/marketbot-daemon-state",
        MARKETBOT_CONFIG_PATH: "/tmp/marketbot-daemon-state/marketbot.json",
        MARKETBOT_GATEWAY_PORT: "19001",
      },
