...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "marketbot doctor",
        managerScriptArgs(manager, "marketbot", ["doctor", "--non-interactive"]),
        gitRoot,
        { MARKETBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
