...
export function triggerMarketBotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MARKETBOT_SYSTEMD_UNIT,
        process.env.MARKETBOT_PROFILE,
      );
  ...
  const label =
    process.env.MARKETBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MARKETBOT_PROFILE);
