...
function candidateBinDirs(opts: EnsureMarketBotPathOpts): string[] {
  ...
  // Bundled macOS app: `marketbot` lives next to the executable (process.execPath).
  try {
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/marketbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "marketbot"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `marketbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMarketBotCliOnPath(opts: EnsureMarketBotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MARKETBOT_PATH_BOOTSTRAPPED)) {
    return;
  }
