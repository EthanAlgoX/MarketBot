...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MARKETBOT_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*marketbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "marketbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "marketbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "marketbot-gw");
  ...
  records.push(`_marketbot-gw._tcp IN PTR ${instanceLabel}._marketbot-gw._tcp`);
  records.push(`${instanceLabel}._marketbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._marketbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; marketbot-content-hash: ${contentHash}\n${contentBody}`;
}
