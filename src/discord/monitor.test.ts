...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of MarketBot")).toBe("friends-of-marketbot");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of MarketBot"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-marketbot" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-marketbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MarketBot"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-marketbot");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-marketbot": { slug: "friends-of-marketbot" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of MarketBot"),
      guildEntries,
...
describe("discord group DM gating", () => {
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["marketbot-dm"],
        channelId: "1",
        channelName: "MarketBot DM",
        channelSlug: "marketbot-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
