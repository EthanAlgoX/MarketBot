...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "MARKETBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MARKETBOT_STATE_DIR = path.join(home, ".marketbot");
    ...
    await fs.mkdir(path.join(home, ".marketbot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.marketbot${path.sep}media`);
