...
- Jobs persist under `~/.marketbot/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.marketbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.marketbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.marketbot/cron/jobs.json",
...
marketbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
marketbot cron edit <jobId> --agent ops
marketbot cron edit <jobId> --clear-agent
```
```
...
```bash
marketbot system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`marketbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `MARKETBOT_SKIP_CRON`.
