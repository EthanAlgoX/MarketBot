...
read_when:
  - "You installed MarketBot but `marketbot` is “command not found”"
...
MarketBot’s runtime baseline is **Node 22+**.

If you can run `npm install -g marketbot@latest` but later see `marketbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
