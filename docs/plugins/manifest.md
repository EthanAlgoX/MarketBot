...
read_when:
  - You are building a MarketBot plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (marketbot.plugin.json)

Every plugin **must** ship a `marketbot.plugin.json` file in the **plugin root**.
MarketBot uses this manifest to validate configuration **without executing plugin
