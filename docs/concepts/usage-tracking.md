...
- `/usage cost` in chats: local cost summary aggregated from MarketBot session logs.
- CLI: `marketbot status --usage` prints a full per-provider breakdown.
- CLI: `marketbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
