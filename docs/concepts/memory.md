---
summary: "How MarketBot memory works (workspace files + automatic memory flush)"
read_when:
...
MarketBot memory is **plain Markdown in the agent workspace**. The files are the
...
When a session is **close to auto-compaction**, MarketBot triggers a **silent,
...
- Uses remote embeddings by default. If `memorySearch.provider` is not set, MarketBot auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. MarketBot
...
When enabled, MarketBot combines:
- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, MarketBot falls back to vector-only search.
...
MarketBot can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.marketbot/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, MarketBot stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, MarketBot logs the
