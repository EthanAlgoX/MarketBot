...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MarketBot
---

# Node + tsx "__name is not a function" crash

## Summary
Running MarketBot via Node with `tsx` fails at startup with:

```
[marketbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `marketbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch marketbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node marketbot.mjs status` works on Node 25.
