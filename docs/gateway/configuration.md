---
summary: "All configuration options for ~/.marketbot/marketbot.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

MarketBot reads an optional **JSON5** config from `~/.marketbot/marketbot.json` (comments + trailing commas allowed).

If the file is missing, MarketBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.marketbot/workspace`). You usually only need a config to:
...
MarketBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `marketbot doctor`, `marketbot logs`, `marketbot health`, `marketbot status`, `marketbot service`, `marketbot help`).
- Run `marketbot doctor` to see the exact issues.
- Run `marketbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `marketbot config set`. Keep a backup of `~/.marketbot/marketbot.json`.
...
```bash
marketbot gateway call config.get --params '{}' # capture payload.hash
marketbot gateway call config.apply --params '{
...
```bash
marketbot gateway call config.get --params '{}' # capture payload.hash
marketbot gateway call config.patch --params '{
...
```json5
{
  agents: {
    ...
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@marketbot", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.marketbot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/.marketbot/workspace" }
  ]
}
```
...
{ "$include": "/etc/marketbot/base.json5" } // absolute
...
```json5
// ~/.marketbot/clients/mueller/agents.json5
[
...
```json5
// ~/.marketbot/clients/mueller/broadcast.json5
{
...
MarketBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, MarketBot runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `MARKETBOT_LOAD_SHELL_ENV=1`
- `MARKETBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
MarketBot stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.marketbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.marketbot/credentials/oauth.json` (or `$MARKETBOT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.marketbot/agent/*` (migrated by `marketbot doctor` into `~/.marketbot/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `MARKETBOT_OAUTH_DIR`
- Agent dir (default agent root override): `MARKETBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, MarketBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, MarketBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/marketbot/marketbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/marketbot/marketbot.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/marketbot/marketbot.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.marketbot/credentials/whatsapp/biz
          // authDir: "~/.marketbot/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `marketbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@marketbot", "marketbot"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@marketbot"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.marketbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.marketbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.marketbot/workspace-personal",
        sandbox: { mode: "off" }
      }
    ]
  }
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.marketbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro"
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.marketbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.marketbot/workspace-home" },
      { id: "work", workspace: "~/.marketbot/workspace-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `marketbot.json`).
...
MarketBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["marketbot-dm"]          // optional group DM allowlist
      },
      guilds: {
...
MarketBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `marketbot plugins install @marketbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
MarketBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
MarketBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, MarketBot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/marketbot" } }
}
```
...
When a file exceeds this limit, MarketBot logs a warning and injects a truncated
...
message envelopes). If unset, MarketBot uses the host timezone at runtime.
...
`[marketbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[marketbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[marketbot]"`, MarketBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, MarketBot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.marketbot/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"MarketBot"`).
...
MarketBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, MarketBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `marketbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `tools.web.fetch.strategy` (`fast` | `waterfall` | `race`, default `waterfall`)
- `tools.web.fetch.maxChars` (default 50000)
- `tools.web.fetch.timeoutSeconds` (default 30)
- `tools.web.fetch.cacheTtlMinutes` (default 15)
- `tools.web.fetch.maxRedirects` (default 3)
- `tools.web.fetch.userAgent` (optional override)
- `tools.web.fetch.readability` (default true; applies to native HTML extraction before fallback engines)
...
- `tools.web.fetch.firecrawl.proxy` (`auto` | `basic` | `stealth`, default `auto`)
- `tools.web.fetch.firecrawl.storeInCache` (default true)
...
- `group:nodes`: `nodes`
- `group:marketbot`: all built-in MarketBot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.marketbot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.marketbot/sandboxes",
        docker: {
          ...
          apparmorProfile: "marketbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "marketbot-sandbox-browser:bookworm-slim",
          containerPrefix: "marketbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
MarketBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.marketbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
MarketBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, MarketBot writes/merges a `models.json` into
`~/.marketbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. MarketBot uses
...
- Shortcut: `marketbot onboard --auth-choice opencode-zen`.
...
Shortcut: `marketbot onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `marketbot onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMI_API_KEY` in the environment or use `marketbot onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `marketbot onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `marketbot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `MARKETBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.marketbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.marketbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.marketbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, MarketBot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.marketbot/skills` (workspace skills
...
from `~/.marketbot/extensions`, `<workspace>/.marketbot/extensions`, plus any
...
### `browser` (marketbot-managed browser)

MarketBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for marketbot and expose a small loopback control service.
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/marketbot" }
...
- Examples: `"/ui"`, `"/marketbot"`, `"/apps/marketbot"`.
...
- When a connection comes from one of these IPs, MarketBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `marketbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `MARKETBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `MARKETBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. MarketBot
...
macOS app behavior:
- MarketBot.app watches `~/.marketbot/marketbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.marketbot/marketbot.json` (or `MARKETBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.marketbot/marketbot.json` (or `MARKETBOT_CONFIG_PATH`)
...
- `MARKETBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `marketbot --dev …` → uses `~/.marketbot-dev` + shifts ports from base `19001`
- `marketbot --profile <name> …` → uses `~/.marketbot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `marketbot webhooks gmail setup` / `run`):
...
- Set `MARKETBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, MarketBot defaults `serve.path` to `/` so
...
Default root: `~/.marketbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the marketbot browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__marketbot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__marketbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.marketbot/workspace/canvas",
    port: 18793,
    liveReload: true
  }
}
...
- env: `MARKETBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.marketbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.marketbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.marketbot/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_marketbot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `marketbot` (advertises `marketbot.local`). Override with `MARKETBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_marketbot-gw._tcp` under `~/.marketbot/dns/` using the configured discovery domain (example: `marketbot.internal.`).
