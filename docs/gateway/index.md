...
marketbot gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
marketbot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.marketbot/marketbot.json` (or `MARKETBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__marketbot__/canvas/` from `~/.marketbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `MARKETBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `MARKETBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `MARKETBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.marketbot.*` may still exist)
- Linux: `marketbot-gateway-<profile>.service`
- Windows: `MarketBot Gateway (<profile>)`

Install metadata is embedded in the service config:
- `MARKETBOT_SERVICE_MARKER=marketbot`
- `MARKETBOT_SERVICE_KIND=gateway`
...
```bash
marketbot --dev setup
marketbot --dev gateway --allow-unconfigured
# then target the dev instance:
marketbot --dev status
marketbot --dev health
```

Defaults (can be overridden via env/flags/config):
- `MARKETBOT_STATE_DIR=~/.marketbot-dev`
- `MARKETBOT_CONFIG_PATH=~/.marketbot-dev/marketbot.json`
- `MARKETBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.marketbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `MARKETBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `MARKETBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `MARKETBOT_CONFIG_PATH`
- unique `MARKETBOT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
marketbot --profile main gateway install
marketbot --profile rescue gateway install
```

Example:
```bash
MARKETBOT_CONFIG_PATH=~/.marketbot/a.json MARKETBOT_STATE_DIR=~/.marketbot-a marketbot gateway --port 19001
MARKETBOT_CONFIG_PATH=~/.marketbot/b.json MARKETBOT_STATE_DIR=~/.marketbot-b marketbot gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `marketbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `marketbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `marketbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.marketbot.*` is cleaned up).
  - `marketbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are MarketBot profile services.
  ...
  - Cleanup: `marketbot gateway uninstall` (current service) and `marketbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `marketbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- MarketBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.marketbot.*` labels still unload cleanly).
- To stop it cleanly, use `marketbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `marketbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `marketbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
MarketBot installs a **systemd user service** by default on Linux/WSL2. We
...
`marketbot gateway install` writes the user unit. `marketbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/marketbot-gateway[-<profile>].service`:
```
[Unit]
Description=MarketBot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/marketbot gateway --port 18789
Restart=always
RestartSec=5
Environment=MARKETBOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now marketbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/marketbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now marketbot-gateway[-<profile>].service
```
...
## CLI helpers
- `marketbot gateway health|status` — request health/status over the Gateway WS.
- `marketbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `marketbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `marketbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `marketbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `marketbot gateway` and the legacy TCP control port.
