...
- During heartbeat runs, MarketBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `marketbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, MarketBot skips the heartbeat run entirely (no model call).
...
```bash
marketbot system event --text "Check for urgent follow-ups" --mode now
```
