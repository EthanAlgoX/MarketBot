---
summary: "Quick troubleshooting guide for common MarketBot failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When MarketBot misbehaves, here's how to fix it.
...
|---|---|---|
| `marketbot status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `marketbot status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `marketbot status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `marketbot gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `marketbot channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `marketbot gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `marketbot logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `marketbot status --all` (it redacts tokens). If you paste `marketbot status`, consider setting `MARKETBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  marketbot models auth setup-token --provider anthropic
  ```
...
```bash
marketbot models auth paste-token --provider anthropic
marketbot models status
```
...
**Logs:**
- Preferred: `marketbot logs --follow`
- File logs (always): `/tmp/marketbot/marketbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$MARKETBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u marketbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  marketbot config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  marketbot config set gateway.mode remote
  marketbot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `marketbot setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.marketbot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `marketbot doctor`
...
- then run `marketbot sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `marketbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `marketbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `marketbot gateway status` and `marketbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `MARKETBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `marketbot gateway status` shows a config mismatch**
...
- Fix: rerun `marketbot gateway install --force` from the same `--profile` / `MARKETBOT_STATE_DIR` you want the service to use.

**If `marketbot gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `marketbot doctor` to update it (or `marketbot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `MARKETBOT_GATEWAY_TOKEN`) and restart the service.

**If `marketbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/marketbot` on disk.
...
Symptoms: `pwd` or file tools show `~/.marketbot/sandboxes/...` even though you
...
MarketBot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `marketbot models list` or
  `marketbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${MARKETBOT_CONFIG_PATH:-$HOME/.marketbot/marketbot.json}"
```
...
tail -f "$(ls -t /tmp/marketbot/marketbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
marketbot pairing list <channel>
```
...
```bash
marketbot logs --follow | grep "pairing request"
```
...
- ❌ `@marketbot` + image
- ✅ `@marketbot check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.marketbot/agents/<agentId>/sessions/
```
...
marketbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
marketbot gateway --verbose
```

If you’re logged out / unlinked:

```bash
marketbot channels logout
trash "${MARKETBOT_STATE_DIR:-$HOME/.marketbot}/credentials" # if logout can't cleanly remove everything
marketbot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/marketbot/marketbot-*.log | head -1)" | tail -20
```

### High Memory Usage

MarketBot keeps conversation history in memory.
...
MarketBot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `marketbot doctor` reports every invalid entry.
- `marketbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `marketbot logs`, `marketbot health`, `marketbot status`, `marketbot gateway status`, and `marketbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/marketbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `marketbot models status` (CLI).
...
```bash
curl -fsSL https://marketbot.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `marketbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. MarketBot scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/marketbot/marketbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.marketbot.* still works)
```
...
Ensure the global `marketbot` CLI is installed and matches the app version:
```bash
marketbot --version
npm install -g marketbot@<version>
```
...
#   ${MARKETBOT_CONFIG_PATH:-$HOME/.marketbot/marketbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
marketbot gateway --verbose
marketbot channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/marketbot/marketbot-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$MARKETBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.marketbot/logs/...`; profiles use `~/.marketbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u marketbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "MarketBot Gateway (<profile>)" /V /FO LIST` |
| Session files | `$MARKETBOT_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$MARKETBOT_STATE_DIR/media/` |
| Credentials | `$MARKETBOT_STATE_DIR/credentials/` |
...
marketbot gateway status --deep

# Is the gateway reachable?
marketbot health --json
# If it fails, rerun with connection details:
marketbot health --verbose
...
tail -20 /tmp/marketbot/marketbot-*.log
```
...
trash "${MARKETBOT_STATE_DIR:-$HOME/.marketbot}"
marketbot channels login         # re-pair WhatsApp
marketbot gateway restart           # or: marketbot gateway
```
...
1. Check logs first: `/tmp/marketbot/` (default: `marketbot-YYYY-MM-DD.log`, or your configured `logging.file`)
