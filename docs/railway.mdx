...
Deploy MarketBot on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/marketbot` — Control UI
...
- `MARKETBOT_STATE_DIR=/data/.marketbot` (recommended)
- `MARKETBOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `MARKETBOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your MarketBot state + workspace so you can migrate to another host without losing config or memory.
