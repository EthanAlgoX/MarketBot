...
Matrix is an open, decentralized messaging protocol. MarketBot connects as a Matrix **user**
...
```bash
marketbot plugins install @marketbot/matrix
```

Local checkout (when running from a git repo):

```bash
marketbot plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `marketbot plugins install @marketbot/matrix`
   - From a local checkout: `marketbot plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MarketBot calls the same
     login endpoint, stores the access token in `~/.marketbot/credentials/matrix/credentials.json`,
...
- On first connection, MarketBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MarketBot logs a warning.
...
`~/.marketbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `marketbot pairing list matrix`
  - `marketbot pairing approve matrix <CODE>`
...
- On startup, MarketBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
