...
read_when:
  - Setting up Twitch chat integration for MarketBot
---
# Twitch (plugin)

Twitch chat support via IRC connection. MarketBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
marketbot plugins install @marketbot/twitch
```

Local checkout (when running from a git repo):

```bash
marketbot plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `MARKETBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "marketbot",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use MARKETBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
MARKETBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
