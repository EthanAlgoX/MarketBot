...
```bash
marketbot plugins install @marketbot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
marketbot plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "MarketBot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `marketbot pairing list nextcloud-talk`
  - `marketbot pairing approve nextcloud-talk <CODE>`
