...
Tlon is a decentralized messenger built on Urbit. MarketBot connects to your Urbit ship and can
...
```bash
marketbot plugins install @marketbot/tlon
```

Local checkout (when running from a git repo):

```bash
marketbot plugins install ./extensions/tlon
```
...
Use these with `marketbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MarketBot replies in-thread.
