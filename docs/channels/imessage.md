...
3) Configure MarketBot with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for MarketBot + `imsg` (Messages DB access).
...
2) Create a macOS user (example: `marketbothome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. MarketBot only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. MarketBot can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, MarketBot attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - marketbot gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.marketbot/scripts/imsg-ssh",
...
Example wrapper (`~/.marketbot/scripts/imsg-ssh`):
```bash
#!/usr/bin/env bash
...
- Approve via:
  - `marketbot pairing list imessage`
  - `marketbot pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, MarketBot treats that thread as a “group” for:
