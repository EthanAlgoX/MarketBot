...
read_when:
  - You want MarketBot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables MarketBot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`marketbot onboard`) and `marketbot channels add` list optional channel plugins.
...
```bash
marketbot plugins install @marketbot/nostr
```

Use a local checkout (dev workflows):

```bash
marketbot plugins install --link <path-to-marketbot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "marketbot@example.com",
        "lud16": "marketbot@example.com"
      }
    }
  }
}
