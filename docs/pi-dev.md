...
This guide summarizes a sane workflow for working on the pi integration in MarketBot.
...
- Trigger the agent directly:
  - `pnpm marketbot agent --message "Hello" --thinking low`
...
State lives under the MarketBot state directory. Default is `~/.marketbot`. If `MARKETBOT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `marketbot.json` for config
...
- https://docs.marketbot.ai/testing
- https://docs.marketbot.ai/start/getting-started
