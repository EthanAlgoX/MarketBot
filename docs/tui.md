...
Remote Gateway:
```bash
marketbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `marketbot tui --deliver`
...
- Check the Gateway logs: `marketbot logs --follow`.
- Confirm the agent can run: `marketbot status` and `marketbot models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `marketbot agents list` and your routing config.
