...
Deploy MarketBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/marketbot/marketbot" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run MarketBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MARKETBOT_STATE_DIR
        value: /data/.marketbot
      - key: MARKETBOT_WORKSPACE_DIR
        value: /data/workspace
      - key: MARKETBOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/marketbot`.
...
If you use the original MarketBot repository, Render will not auto-deploy your MarketBot. To update it, run a manual Blueprint sync from the dashboard.
...
For MarketBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any MarketBot host.
