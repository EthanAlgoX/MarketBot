---
summary: "Use Venice AI privacy-focused models in MarketBot"
read_when:
  - You want privacy-focused inference in MarketBot
...
```bash
marketbot onboard --auth-choice venice-api-key
```
...
```bash
marketbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, MarketBot shows all available Venice models. Pick based on your needs:
...
```bash
marketbot models set venice/claude-opus-45
marketbot models set venice/llama-3.3-70b
```

List all available models:

```bash
marketbot models list | grep venice
```

## Configure via `marketbot configure`

1. Run `marketbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
MarketBot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
marketbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
marketbot chat --model venice/claude-opus-45

# Use uncensored model
marketbot chat --model venice/venice-uncensored

# Use vision model with image
marketbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
marketbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `marketbot models list` to see currently available models. Some models may be temporarily offline.
