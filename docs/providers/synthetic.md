---
summary: "Use Synthetic's Anthropic-compatible API in MarketBot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. MarketBot registers it as the
...
```bash
marketbot onboard --auth-choice synthetic-api-key
```
...
Note: MarketBot's Anthropic client appends `/v1` to the base URL, so use
