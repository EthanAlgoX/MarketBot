---
summary: "Model providers (LLMs) supported by MarketBot"
read_when:
...
MarketBot can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `marketbot onboard`).
