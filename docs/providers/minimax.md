---
summary: "Use MiniMax M2.1 in MarketBot"
read_when:
  - You want MiniMax models in MarketBot
...
```bash
marketbot plugins enable minimax-portal-auth  # skip if already loaded.
marketbot gateway restart  # restart if gateway is already running
marketbot onboard --auth-choice minimax-portal
```
You will be prompted to select an endpoint:
- **Global** - International users (`api.minimax.io`)
- **CN** - Users in China (`api.minimaxi.com`)

See [MiniMax OAuth plugin README](https://github.com/marketbot/marketbot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `marketbot.json`:
...
- Use `marketbot models list` and `marketbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `marketbot configure` and selecting **MiniMax M2.1**, or
