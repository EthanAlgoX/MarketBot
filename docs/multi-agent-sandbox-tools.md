...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `marketbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `marketbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "name=marketbot-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${MARKETBOT_STATE_DIR:-$HOME/.marketbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
