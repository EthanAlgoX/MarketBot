---
summary: "Chrome extension: let MarketBot drive your existing Chrome tab"
read_when:
...
The MarketBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate marketbot-managed Chrome profile.
...
MarketBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MarketBot release (npm package) as static files. There is no separate “build” step.

After upgrading MarketBot:
- Re-run `marketbot browser extension install` to refresh the installed files under your MarketBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

MarketBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want MarketBot to control.
...
- `ON`: attached; MarketBot can drive that tab.
...
Debugging: `marketbot sandbox explain`
...
`marketbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `marketbot browser extension install` first to copy the extension to a stable location under your MarketBot state directory.
...
- **This is not isolated** like the dedicated marketbot-managed profile.
