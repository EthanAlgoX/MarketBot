---
summary: "Agent tool surface for MarketBot (browser, canvas, nodes, message, cron) replacing legacy `marketbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `marketbot-*` skills
---

# Tools (MarketBot)

MarketBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `marketbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `marketbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, MarketBot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`marketbot node run`).
...
Notes:
- Requires a Brave API key (recommended: `marketbot configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated MarketBot-managed browser.
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to "marketbot").
...
- Quick smoke: `marketbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `marketbot gateway` restart in-place)
