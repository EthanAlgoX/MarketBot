...
- **gateway host** → `marketbot` process on the gateway machine
...
`~/.marketbot/exec-approvals.json` directly.

CLI: `marketbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
