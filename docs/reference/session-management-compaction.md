...
This document explains how MarketBot manages sessions end-to-end:
...
MarketBot is designed around a single **Gateway process** that owns session state.
...
MarketBot persists sessions in two layers:
...
- Transcripts: `~/.marketbot/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

MarketBot resolves these via `src/config/sessions.ts`.
...
MarketBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (MarketBot consumes the events, but Pi decides when to compact).
...
MarketBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, MarketBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, MarketBot leaves it alone.
...
- `marketbot status` (CLI)
- `marketbot sessions` / `sessions --json`
...
MarketBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- MarketBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, MarketBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
MarketBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but MarketBot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `marketbot status`.
