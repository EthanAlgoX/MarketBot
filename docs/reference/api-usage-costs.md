...
MarketBot features that can generate provider usage or paid API calls.
...
- `marketbot status --usage` and `marketbot channels list` show provider **usage windows**
...
MarketBot can pick up credentials from:
...
- `marketbot status --usage`
- `marketbot models status --json`
...
### 8) Model scan / probe
`marketbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
