---
summary: "First-run onboarding flow for MarketBot (macOS app)"
read_when:
...
- Writes credentials to `~/.marketbot/credentials/oauth.json`
...
The app can install the global `marketbot` CLI via npm/pnpm so terminal
...
On the first agent run, MarketBot bootstraps a workspace (default `~/.marketbot/workspace`):
...
```bash
marketbot webhooks gmail setup --account you@gmail.com
```
...
- `~/.marketbot/credentials/oauth.json`
