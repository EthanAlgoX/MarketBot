---
summary: "Research notes: offline memory system for MarketBot workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.marketbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep MarketBot integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: MarketBot-style workspace (`agents.defaults.workspace`, default `~/.marketbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.marketbot/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want MarketBot to see every session.
...
```
~/.marketbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in MarketBot**, but keep a separable core library.

### Why integrate into MarketBot?
- MarketBot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `marketbot memory recall "…" --k 25 --since 30d`
  - `marketbot memory reflect --since 7d`
...
Pragmatic take for `~/.marketbot/workspace`:
