...
Delivery:
- Publish as `marketbot/plugin-sdk` (or export from core under `marketbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `MarketBotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: MarketBotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: MarketBotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: MarketBotConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: MarketBotConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: MarketBotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: MarketBotConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `marketbot/plugin-sdk`.
- Add `api.runtime` to `MarketBotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `marketbotRuntime: ">=2026.2.0"`).
