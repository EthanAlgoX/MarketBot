...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.marketbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.marketbot/node.json` (node id + pairing token).
...
Path: `~/.marketbot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.marketbot/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.marketbot/exec-approvals.sock` (0600).
