...
read_when:
  - Installing MarketBot on Windows
  - Looking for Windows companion app status
---
# Windows (WSL2)

MarketBot on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `marketbot status --all` to confirm.
...
### 3) Install MarketBot (inside WSL)
