---
summary: "MarketBot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up MarketBot on a Raspberry Pi
  - Running MarketBot on ARM devices
  - Building a cheap always-on personal AI
---

# MarketBot on Raspberry Pi

## Goal

Run a persistent, always-on MarketBot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most MarketBot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled marketbot
...
```bash
# Check logs
journalctl -u marketbot --no-pager -n 100

# Common fix: rebuild
cd ~/marketbot  # if using hackable install
npm run build
sudo systemctl restart marketbot
```
