...
service via the external `marketbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`MARKETBOT_PROFILE`; legacy `com.marketbot.*` is supported).
