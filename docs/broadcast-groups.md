...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when MarketBot would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When MarketBot would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/marketbot-alfred/
...
Workspace: /Users/pascal/marketbot-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.marketbot/logs/gateway.log | grep broadcast
```
