...
You can also set `MARKETBOT_VERBOSE=1` instead of the flag.
...
### `docs.marketbot.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.marketbot.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.marketbot.ai` to the allowlist, then retry.
...
- Check the allowlist: `marketbot config get agents.defaults.models`
...
If you can, include the relevant log tail from `marketbot logs --follow`.
