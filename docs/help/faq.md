---
summary: "Frequently asked questions about MarketBot setup, configuration, and usage"
---
# FAQ
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What’s the recommended way to install and set up MarketBot?](#whats-the-recommended-way-to-install-and-set-up-marketbot)
  ...
  - [I can't access docs.marketbot.ai (SSL error). What now?](#i-cant-access-docsmarketbotai-ssl-error-what-now)
  - [What’s the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or marketbot not recognized](#windows-install-says-git-not-found-or-marketbot-not-recognized)
  - [The docs didn’t answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install MarketBot on Linux?](#how-do-i-install-marketbot-on-linux)
  - [How do I install MarketBot on a VPS?](#how-do-i-install-marketbot-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask MarketBot to update itself?](#can-i-ask-marketbot-to-update-itself)
  ...
  - [If I buy a Mac mini to run MarketBot, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-marketbot-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different MarketBot instances?](#can-multiple-people-use-one-whatsapp-number-with-different-marketbot-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What’s the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run MarketBot on a dedicated machine?](#how-important-is-it-to-run-marketbot-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run MarketBot in a VM and what are the requirements](#can-i-run-marketbot-in-a-vm-and-what-are-the-requirements)
- [What is MarketBot?](#what-is-marketbot)
  - [What is MarketBot, in one paragraph?](#what-is-marketbot-in-one-paragraph)
  - [What’s the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for MarketBot](#what-are-the-top-five-everyday-use-cases-for-marketbot)
  - [Can MarketBot help with lead gen outreach ads and blogs for a SaaS](#can-marketbot-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with MarketBot saved locally?](#is-all-data-used-with-marketbot-saved-locally)
  - [Where does MarketBot store its data?](#where-does-marketbot-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What’s the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall MarketBot?](#how-do-i-completely-uninstall-marketbot)
...
- [Config basics](#config-basics)
  ...
  - [Can the MarketBot browser run headless?](#can-the-marketbot-browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two MarketBot instances talk to each other (local + VPS)?](#can-two-marketbot-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does MarketBot load environment variables?](#how-does-marketbot-load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of MarketBot instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-marketbot-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset MarketBot but keep it installed?](#how-do-i-completely-reset-marketbot-but-keep-it-installed)
  ...
  - [Why doesn’t MarketBot reply in a group?](#why-doesnt-marketbot-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do MarketBot, Flawd, and Krill use for models?](#what-do-marketbot-flawd-and-krill-use-for-models)
...
- [Gateway: ports, “already running”, and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `marketbot gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-marketbot-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `marketbot gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-marketbot-gateway-status-show-config-cli-and-config-service-different)
  - [What does “another gateway instance is already listening” mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run MarketBot in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-marketbot-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart MarketBot?](#i-closed-my-terminal-on-windows-how-do-i-restart-marketbot)
  ...
  - [ELI5: `marketbot gateway restart` vs `marketbot gateway`](#eli5-marketbot-gateway-restart-vs-marketbot-gateway)
  - [What’s the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose MarketBot to inbound DMs?](#is-it-safe-to-expose-marketbot-to-inbound-dms)
...
2) **Pasteable report (safe to share)**
   ```bash
   marketbot status --all
   ```
   Read-only diagnosis with log tail (tokens redacted).

3) **Daemon + port state**
   ```bash
   marketbot gateway status
   ```
   Shows supervisor runtime vs RPC reachability, the probe target URL, and which config the service likely used.

4) **Deep probes**
   ```bash
   marketbot status --deep
   ```
...
5) **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/marketbot/marketbot-*.log | head -1)"
   ```
...
7) **Gateway snapshot**
   ```bash
   marketbot health --json
   marketbot health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://marketbot.bot/install.sh | bash -s -- --install-method git
```

This installs MarketBot **from a git checkout**, so the agent can read the code + docs and
...
https://github.com/marketbot/marketbot/issues
https://github.com/marketbot/marketbot/pulls

Start with these commands (share outputs when asking for help):

```bash
marketbot status
marketbot models status
marketbot doctor
```

What they do:
- `marketbot status`: quick snapshot of gateway/agent health + basic config.
- `marketbot models status`: checks provider auth + model availability.
- `marketbot doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `marketbot status --all`, `marketbot logs --follow`,
`marketbot gateway status`, `marketbot health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up MarketBot

The repo recommends running from source and using the onboarding wizard:

```bash
curl -fsSL https://marketbot.bot/install.sh | bash
...
If you don’t have a global install yet, run it via `pnpm marketbot onboard`.
...
- If it asks for auth, run `marketbot dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `MARKETBOT_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**
- **Tailscale Serve** (recommended): keep bind loopback, run `marketbot gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `marketbot gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from `marketbot dashboard`.
...
1) Install MarketBot on the new machine.
2) Copy `$MARKETBOT_STATE_DIR` (default: `~/.marketbot`) from the old machine.
3) Copy your workspace (default: `~/.marketbot/workspace`).
4) Run `marketbot doctor` and restart the Gateway service.
...
under `~/.marketbot/` (for example `~/.marketbot/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-marketbot-store-its-data),
...
https://github.com/marketbot/marketbot/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.marketbot.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.marketbot.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.marketbot.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsmarketbotai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: https://spa.xfinity.com/check_url_status.

If you still can't reach the site, the docs are mirrored on GitHub:
https://github.com/marketbot/marketbot/tree/main/docs
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://marketbot.bot/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://marketbot.bot/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
https://marketbot.ai/install.ps1
...
```bash
curl -fsSL https://marketbot.bot/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://marketbot.bot/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://marketbot.bot/install.sh | bash -s -- --install-method git --verbose
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or marketbot not recognized
...
**2) marketbot is not recognized after install**
...
### How do I install MarketBot on Linux
...
### How do I install MarketBot on a VPS
...
### Can I ask MarketBot to update itself
...
```bash
marketbot update
marketbot update status
marketbot update --channel stable|beta|dev
marketbot update --tag <dist-tag|version>
marketbot update --no-restart
```

If you must automate from an agent:

```bash
marketbot update --yes --no-restart
marketbot gateway restart
```
...
`marketbot onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run MarketBot with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `marketbot models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `marketbot models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `marketbot models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro/Max)

Yes — via **setup-token**. MarketBot no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so MarketBot can keep replying while a provider is rate‑limited.
...
MarketBot supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. MarketBot fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `marketbot.json`.

Steps:
1) Enable the plugin: `marketbot plugins enable google-gemini-cli-auth`
2) Login: `marketbot models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. MarketBot needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. MarketBot runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
any Mac works. MarketBot’s iMessage integrations run on macOS (BlueBubbles or `imsg`), while
...
### If I buy a Mac mini to run MarketBot can I connect it to my MacBook Pro
...
- Use `marketbot nodes status` / `marketbot nodes list` to see it.
...
- DM your bot, then run `marketbot logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different MarketBot instances
...
If you run MarketBot via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non‑login shells.
...
This **does not delete your data** - it only changes the MarketBot code install. Your state
(`~/.marketbot`) and workspace (`~/.marketbot/workspace`) stay untouched.
...
**MarketBot-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run MarketBot on a dedicated machine
...
MarketBot is lightweight. For a basic Gateway + one chat channel:
...
### Can I run MarketBot in a VM and what are the requirements
...
### What is MarketBot in one paragraph

MarketBot is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

MarketBot is not “just a Claude wrapper.” It’s a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for MarketBot
...
### Can MarketBot help with lead gen outreach ads and blogs for a SaaS
...
MarketBot draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

MarketBot is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use MarketBot when you
...
Showcase: https://marketbot.ai/showcase

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.marketbot/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.marketbot/marketbot.json`). Precedence is `<workspace>/skills` > `~/.marketbot/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.marketbot/marketbot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.marketbot/skills` → bundled → `skills.load.extraDirs`. `markethub` installs into `./skills` by default, which MarketBot treats as `<workspace>/skills`.
...
Checklist:
- Confirm cron is enabled (`cron.enabled`) and `MARKETBOT_SKIP_CRON` is not set.
- Check the Gateway is running 24/7 (no sleep/restarts).
- Verify timezone settings for the job (`--tz` vs host timezone).

Debug:
```bash
marketbot cron run <jobId> --force
marketbot cron runs --id <jobId> --limit 50
```
...
Use **MarketHub** (CLI) or drop skills into your workspace. The macOS Skills UI isn’t available on Linux.
Browse skills at https://markethub.com.

Install the MarketHub CLI (pick one package manager):
...
### Can MarketBot run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.marketbot.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**  
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-marketbot-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**  
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. MarketBot can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3) Override the skill metadata (workspace or `~/.marketbot/skills`) to allow Linux:
   ```markdown
   ---
   name: imsg
   description: iMessage/SMS CLI for listing chats, history, watch, and sending.
   metadata: {"marketbot":{"os":["darwin","linux"],"requires":{"bins":["imsg"]}}}
   ---
   ```
4) Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (MarketBot only needs stdio). See [iMessage](/channels/imessage).
...
MarketBot memory is just Markdown files in the agent workspace:
- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

MarketBot also runs a **silent pre-compaction memory flush** to remind the model
...
If you don’t set a provider explicitly, MarketBot auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, MarketBot
prefers `local`.
...
### Is all data used with MarketBot saved locally

No - **MarketBot’s state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.marketbot` + your workspace directory).
...
### Where does MarketBot store its data

Everything lives under `$MARKETBOT_STATE_DIR` (default: `~/.marketbot`):

| Path | Purpose |
|------|---------|
| `$MARKETBOT_STATE_DIR/marketbot.json` | Main config (JSON5) |
| `$MARKETBOT_STATE_DIR/credentials/oauth.json` | Legacy OAuth import (copied into auth profiles on first use) |
| `$MARKETBOT_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth + API keys) |
| `$MARKETBOT_STATE_DIR/agents/<agentId>/agent/auth.json` | Runtime auth cache (managed automatically) |
| `$MARKETBOT_STATE_DIR/credentials/` | Provider state (e.g. `whatsapp/<accountId>/creds.json`) |
| `$MARKETBOT_STATE_DIR/agents/` | Per‑agent state (agentDir + sessions) |
| `$MARKETBOT_STATE_DIR/agents/<agentId>/sessions/` | Conversation history & state (per agent) |
| `$MARKETBOT_STATE_DIR/agents/<agentId>/sessions/sessions.json` | Session metadata (per agent) |

Legacy single‑agent path: `~/.marketbot/agent/*` (migrated by `marketbot doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.marketbot/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.marketbot`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.marketbot`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.marketbot/skills`).

Default workspace is `~/.marketbot/workspace`, configurable via:

```json5
{
  agents: { defaults: { workspace: "~/.marketbot/workspace" } }
}
```
...
Do **not** commit anything under `~/.marketbot` (credentials, sessions, tokens).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall MarketBot
...
`workspace` to the repo root. The MarketBot repo is just source code; keep the
...
MarketBot reads an optional **JSON5** config from `$MARKETBOT_CONFIG_PATH` (default: `~/.marketbot/marketbot.json`):

```
$MARKETBOT_CONFIG_PATH
```

If the file is missing, it uses safe‑ish defaults (including a default workspace of `~/.marketbot/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `MARKETBOT_GATEWAY_TOKEN`).
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `marketbot doctor --generate-gateway-token`.
...
key. **Recommended:** run `marketbot configure --section web` to store it in
...
- Daemons read env vars from `~/.marketbot/.env` (or the service environment).
...
### Can the MarketBot browser run headless
...
5) Approve the node on the Gateway:
   ```bash
   marketbot nodes pending
   marketbot nodes approve <requestId>
   ```
...
Check the basics:
- Gateway is running: `marketbot gateway status`
- Gateway health: `marketbot status`
- Channel health: `marketbot channels status`
...
### Can two MarketBot instances talk to each other local VPS
...
`marketbot agent --message ... --deliver`, targeting a chat where the other bot
listens. If one bot is on a remote VPS, point your CLI at that remote Gateway
via SSH/Tailscale (see [Remote access](/gateway/remote)).

Example pattern (run from a machine that can reach the target Gateway):
```bash
marketbot agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
Recover:
- Restore from backup (git or a copied `~/.marketbot/marketbot.json`).
- If you have no backup, re-run `marketbot doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:
- Use `marketbot config set` for small changes.
- Use `marketbot configure` for interactive edits.
...
```bash
marketbot gateway --tailscale serve
```
...
### How does MarketBot load environment variables

MarketBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.marketbot/.env` (aka `$MARKETBOT_STATE_DIR/.env`)
...
1) Put the missing keys in `~/.marketbot/.env` so they’re picked up even when the service doesn’t inherit your shell env.
...
`MARKETBOT_LOAD_SHELL_ENV=1`, `MARKETBOT_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`marketbot models status` reports whether **shell env import** is enabled. “Shell env: off”
does **not** mean your env vars are missing - it just means MarketBot won’t load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won’t inherit your shell
environment. Fix by doing one of these:

1) Put the token in `~/.marketbot/.env`:
   ```
...
### Is there a way to make a team of MarketBot instances one CEO and many agents
...
### How do I completely reset MarketBot but keep it installed
...
```bash
marketbot reset --scope full --yes --non-interactive
```
...
- If you used profiles (`--profile` / `MARKETBOT_PROFILE`), reset each state dir (defaults are `~/.marketbot-<profile>`).
- Dev reset: `marketbot gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), MarketBot skips the heartbeat run to save API calls.
...
No. MarketBot runs on **your own account**, so if you’re in the group, MarketBot can see it.
...
```bash
marketbot directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt MarketBot reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.marketbot/agents/<agentId>/sessions/`.
...
- Use `marketbot doctor` to spot stray workspaces and profile mismatches.
...
MarketBot’s default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, MarketBot currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `marketbot models set ...` (updates just model config)
- `marketbot configure --section models` (interactive)
- edit `agents.defaults.model` in `~/.marketbot/marketbot.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `marketbot doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do MarketBot, Flawd, and Krill use for models

- **MarketBot + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-5`) - see [Anthropic](/providers/anthropic).
...
Yes. MarketBot ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
```
~/.marketbot/agents/<agentId>/agent/auth-profiles.json
```

Fix options:
- Run `marketbot agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so MarketBot can keep responding even when a provider is rate‑limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.marketbot/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.marketbot/agent/*` (migrated by `marketbot doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.marketbot/.env` or enable `env.shellEnv`.
- **Make sure you’re editing the correct agent**
  - Multi‑agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity‑check model/auth status**
  - Use `marketbot models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `marketbot models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `marketbot models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.marketbot/.env` on the **gateway host**.
  - Clear any pinned order that forces a missing profile:
    ```bash
    marketbot models auth order clear --provider anthropic
    ```
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), MarketBot will try it during model fallback. If you haven’t configured Google credentials, you’ll see `No API key found for provider "google"`.
...
Fix: MarketBot now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
MarketBot uses provider‑prefixed IDs like:
...
MarketBot may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `marketbot models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

You can also set a **per-agent** order override (stored in that agent’s `auth-profiles.json`) via the CLI:

```bash
# Defaults to the configured default agent (omit --agent)
marketbot models auth order get --provider anthropic

# Lock rotation to a single profile (only try this one)
marketbot models auth order set --provider anthropic anthropic:default

# Or set an explicit order (fallback within provider)
marketbot models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
marketbot models auth order set --provider anthropic --agent main anthropic:default
```

### OAuth vs API key whats the difference

MarketBot supports both:
...
```
--port > MARKETBOT_GATEWAY_PORT > gateway.port > default 18789
```

### Why does marketbot gateway status say Runtime running but RPC probe failed

Because “running” is the **supervisor’s** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `marketbot gateway status` and trust these lines:
- `Probe target:` (the URL the probe actually used)
- `Listening:` (what’s actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn’t listening)

### Why does marketbot gateway status show Config cli and Config service different

You’re editing one config file while the service is running another (often a `--profile` / `MARKETBOT_STATE_DIR` mismatch).

Fix:
```bash
marketbot gateway install --force
```
Run that from the same `--profile` / environment you want the service to use.

### What does another gateway instance is already listening mean

MarketBot enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `marketbot gateway --port <port>`.

### How do I run MarketBot in remote mode client connects to a Gateway elsewhere
...
Notes:
- `marketbot gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
Facts (from code):
- The Control UI stores the token in browser localStorage key `marketbot.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:
- Fastest: `marketbot dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `marketbot doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...`.
- Set `gateway.auth.token` (or `MARKETBOT_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token (or refresh with a one-time `?token=...` link).
- Still stuck? Run `marketbot status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `MARKETBOT_CONFIG_PATH` (per‑instance config)
- `MARKETBOT_STATE_DIR` (per‑instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):
- Use `marketbot --profile <name> …` per instance (auto-creates `~/.marketbot-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `marketbot --profile <name> gateway install`.

Profiles also suffix service names (`bot.molt.<profile>`; legacy `com.marketbot.*`, `marketbot-gateway-<profile>.service`, `MarketBot Gateway (<profile>)`).
...
```
marketbot tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/marketbot/marketbot-YYYY-MM-DD.log
```
...
- macOS: `$MARKETBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.marketbot/logs/...`; profiles use `~/.marketbot-<profile>/logs/...`)
- Linux: `journalctl --user -u marketbot-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "MarketBot Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `marketbot gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart MarketBot
...
1) Is the Gateway running? `marketbot gateway status`
2) Is the Gateway healthy? `marketbot status`
3) Does the UI have the right token? `marketbot dashboard`
...
```bash
marketbot channels status
marketbot channels logs --channel telegram
```
...
### ELI5 marketbot gateway restart vs marketbot gateway

- `marketbot gateway restart`: restarts the **background service** (launchd/systemd).
- `marketbot gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `marketbot gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [MarketBot assistant setup](/start/marketbot) and [Agent send](/tools/agent-send).

CLI sending:

```bash
marketbot message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose MarketBot to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM‑capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `marketbot pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `marketbot pairing list <channel>` if a code didn’t arrive.
- Opening DMs publicly requires explicit opt‑in (`dmPolicy: "open"` and allowlist `"*"`).

Run `marketbot doctor` to surface risky DM policies.
...
```bash
marketbot pairing list telegram
```
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. MarketBot only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
marketbot pairing approve whatsapp <code>
```

List pending requests:

```bash
marketbot pairing list whatsapp
```
...
MarketBot blocks **cross‑provider** messaging by default. If a tool call is bound
...
**A:** In MarketBot, credentials and model selection are separate. Setting `ANTHROPIC_API_KEY` (or storing an Anthropic API key in auth profiles) enables authentication, but the actual default model is whatever you configure in `agents.defaults.model.primary` (for example, `anthropic/claude-sonnet-4-5` or `anthropic/claude-opus-4-5`). If you see `No credentials found for profile "anthropic:default"`, it means the Gateway couldn’t find Anthropic credentials in the expected `auth-profiles.json` for the agent that’s running.

---

Still stuck? Ask in [Discord](https://discord.com/invite/marketbot) or open a [GitHub discussion](https://github.com/marketbot/marketbot/discussions).
