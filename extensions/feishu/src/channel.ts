import type { ChannelPlugin, MarketBotConfig } from "marketbot/plugin-sdk";
import { DEFAULT_ACCOUNT_ID, PAIRING_APPROVED_MESSAGE } from "marketbot/plugin-sdk";
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount> = {
  ...
  agentPrompt: {
    messageToolHints: () => [
      ...
      "- Never output internal trace/debug blocks to users (e.g. `/thinking`, `[web_search_result]`, `[message_id: ...]`, raw tool chunks).",
      "- If search/tool calls fail (e.g. API key missing), do not output raw English error text. Reply in Chinese with a concise fallback and next action.",
  ...
  config: {
    ...
    deleteAccount: ({ cfg }) => {
      const next = { ...cfg } as MarketBotConfig;
