...
MarketBot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
marketbot channels login --channel zalouser
...
```bash
marketbot message send --channel zalouser --target <threadId> --message "Hello from MarketBot!"
```
...
```bash
marketbot channels login --channel zalouser              # Login via QR
marketbot channels login --channel zalouser --account work
marketbot channels status --probe
marketbot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
marketbot directory self --channel zalouser
marketbot directory peers list --channel zalouser --query "name"
marketbot directory groups list --channel zalouser --query "work"
marketbot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
marketbot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
marketbot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `marketbot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
marketbot channels login --channel zalouser --account work
marketbot message send --channel zalouser --account work --target <id> --message "Hello"
