...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["marketbotPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["marketbot", "pollId"]) ??
    readNestedString(value, ["marketbot", "poll", "id"]) ??
    readNestedString(value, ["data", "marketbotPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "marketbot", "pollId"]);
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          marketbotPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "marketbot poll vote",
          displayText: "Vote recorded",
          value: { marketbotPollId: pollId, pollId },
        },
      },
    ],
  };
