...
- **Task tool** in the upstream spec == MarketBot `sessions_spawn`
- **File I/O** == MarketBot `read`/`write`
- **Remote fetch** == MarketBot `web_fetch` (or `exec` with curl when POST is required)
