...
`marketbot.invoke --each`) without adding new MarketBot code per workflow.
...
This extension depends on MarketBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MarketBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.marketbot/extensions` as a standalone plugin directory.
