/**
 * Twitch channel plugin for MarketBot.
 *
...
import { buildChannelConfigSchema } from "marketbot/plugin-sdk";
 ...
 * for MarketBot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MarketBotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MarketBotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MarketBotConfig): boolean => {
