...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "marketbot-nostr-"));
  ...
  try {
    ...
    if (previous === undefined) delete process.env.MARKETBOT_STATE_DIR;
    else process.env.MARKETBOT_STATE_DIR = previous;
