...
function resolveMarketBotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.MARKETBOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set MARKETBOT_ROOT to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveMarketBotRoot();
