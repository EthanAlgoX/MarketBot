...
# MarketBot Auth Widget for Termux
...
# Check auth status
termux-toast "Checking MarketBot auth..."

STATUS=$(ssh "$SERVER" '$HOME/marketbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/marketbot/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|MARKETBOT_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/marketbot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|MARKETBOT_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: MarketBot won't work until re-auth"
                ;;
        esac
        ;;
