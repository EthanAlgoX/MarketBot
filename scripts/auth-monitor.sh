...
#
# Suggested cron: */30 * * * * /home/admin/marketbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MarketBot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., marketbot-alerts)
...
STATE_FILE="$HOME/.marketbot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MarketBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use marketbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MarketBot to $NOTIFY_PHONE..."
            marketbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MarketBot is down. Run: ssh l36 '~/marketbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
