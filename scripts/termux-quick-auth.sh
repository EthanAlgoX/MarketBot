...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/marketbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MarketBot Re-Auth" -c "After getting key, run: ssh $SERVER '~/marketbot/scripts/mobile-reauth.sh'" --id marketbot-auth
        ;;
    *)
