...
function trySetupCompletion(repoRoot) {
  ...
  if (process.env.CI || process.env.MARKETBOT_SKIP_COMPLETION_SETUP) return;

  const binPath = path.join(repoRoot, 'marketbot.mjs');
  ...
  try {
    // Run with MARKETBOT_SKIP_POSTINSTALL to avoid any weird recursion,
    // though distinct from this script.
    spawnSync(process.execPath, [binPath, 'completion', '--install', '--yes'], {
      cwd: repoRoot,
      stdio: 'inherit',
      env: { ...process.env, MARKETBOT_SKIP_POSTINSTALL: '1' },
    });
  } catch (err) {
...
try {
  const skip =
    process.env.MARKETBOT_SKIP_POSTINSTALL === '1' ||
