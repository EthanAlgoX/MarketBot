...
# Build and bundle MarketBot into a minimal .app we can open.
# Outputs to dist/MarketBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/MarketBot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MarketBot"
BUNDLE_ID="${BUNDLE_ID:-ai.marketbot.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/marketbot/marketbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MarketBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MarketBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MarketBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MarketBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MarketBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MarketBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MarketBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MarketBot/Resources/MarketBot.icns" "$APP_ROOT/Contents/Resources/MarketBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MarketBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MarketBotKit resources"
MARKETBOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MarketBotKit_MarketBotKit.bundle"
if [ -d "$MARKETBOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MarketBotKit_MarketBotKit.bundle"
  cp -R "$MARKETBOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MarketBotKit_MarketBotKit.bundle"
else
  echo "WARN: MarketBotKit resource bundle not found at $MARKETBOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running MarketBot"
killall -q MarketBot 2>/dev/null || true
