#!/bin/bash
# Setup MarketBot Auth Management System
...
echo "=== MarketBot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'marketbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MarketBot message: Send warning via MarketBot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/marketbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/marketbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/marketbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now marketbot-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/MarketBotAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/MarketBotAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/MarketBot*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (MarketBotAuth) shows status and opens auth URL if needed."
echo "The full widget (MarketBotAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status marketbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
