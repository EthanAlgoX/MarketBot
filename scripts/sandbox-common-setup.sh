...
BASE_IMAGE="${BASE_IMAGE:-marketbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-marketbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=marketbot.sandbox=1)
NOTE
