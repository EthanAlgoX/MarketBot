...
IMAGE_NAME="marketbot-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="marketbot-net-e2e-$$"
GW_NAME="marketbot-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MARKETBOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "MARKETBOT_SKIP_CHANNELS=1" \
	  -e "MARKETBOT_SKIP_GMAIL_WATCHER=1" \
	  -e "MARKETBOT_SKIP_CRON=1" \
	  -e "MARKETBOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
