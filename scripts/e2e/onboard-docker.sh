...
IMAGE_NAME="marketbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/marketbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/marketbot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/marketbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/marketbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/marketbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/marketbot"
    config_path="$HOME/.marketbot/marketbot.json"
    sessions_dir="$HOME/.marketbot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.marketbot/marketbot.json" <<'"'"'JSON'"'"'
{
