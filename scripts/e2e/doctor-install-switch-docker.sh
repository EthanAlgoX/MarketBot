...
IMAGE_NAME="marketbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/marketbot-bin:$PATH"
  mkdir -p /tmp/marketbot-bin

  cat > /tmp/marketbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/marketbot-bin/systemctl

  cat > /tmp/marketbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/marketbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/marketbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/marketbot/marketbot.mjs"
  git_entry="/app/marketbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/marketbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/marketbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
