...
INSTALL_URL="${MARKETBOT_INSTALL_URL:-https://marketbot.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MARKETBOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MARKETBOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="marketbot"
PACKAGE_NAME="${MARKETBOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MARKETBOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MARKETBOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MARKETBOT_INSTALL_LATEST_OUT:-}"
fi
