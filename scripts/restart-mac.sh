#!/usr/bin/env bash
# Reset MarketBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${MARKETBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="MarketBot.app/Contents/MacOS/MarketBot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/MarketBot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/MarketBot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/MarketBot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.marketbot.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/marketbot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${MARKETBOT_RESTART_LOG:-/tmp/marketbot-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${MARKETBOT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.marketbot/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  MARKETBOT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node marketbot.mjs daemon install --force --runtime node"
      log "  node marketbot.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.marketbot/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.marketbot.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing MarketBot instances"
kill_all_marketbot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product MarketBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/MarketBot.app" ]]; then
    APP_BUNDLE="/Applications/MarketBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/MarketBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/MarketBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/MarketBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set MARKETBOT_APP_BUNDLE to your installed MarketBot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node marketbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node marketbot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".marketbot", "marketbot.json"), "utf8");
