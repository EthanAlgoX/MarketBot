...
ZIP=${1:?"Usage: $0 MarketBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/marketbot/marketbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^MarketBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/marketbot/marketbot/releases/download/v${VERSION}/"}
